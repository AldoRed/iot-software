...
static void set_servo_angle(uint32_t angle) {
  ...
  ESP_LOGI(TAG, "Angulo: %d - Pulso: %u us - Duty: %u", (int)angle,
           (unsigned int)pulse_width, (unsigned int)duty);
...
void app_main(void) {
  ...
  // Bucle principal: se mueve el servo a 0°, luego 90° y finalmente 180° con 5
  // segundos de espera en cada posición
  while (1) {
    set_servo_angle(0);
