...
typedef enum { STATE_RED, STATE_YELLOW, STATE_GREEN } TrafficState;
...
void app_main(void) {
  ...
  while (1) {
    ...
    if (current_button_state == 1 && last_button_state == 0) {
      ...
      // Si la pulsación es dentro del intervalo establecido, se acumula el
      // conteo, de lo contrario se reinicia el contador.
    ...
    // Verificar si pasó el intervalo sin nueva pulsación para ejecutar la
    // acción correspondiente
    if ((xTaskGetTickCount() - last_press_time) >= PRESS_INTERVAL &&
        press_count > 0) {
      if (press_count == 2) {
        advance_state();
